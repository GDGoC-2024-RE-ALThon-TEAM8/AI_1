...
# 2. M4A 파일을 WAV로 변환하는 함수
async def convert_m4a_to_wav(audio_file):
    # M4A 파일을 읽어서 WAV 포맷으로 변환
    audio_bytes = await audio_file.read()  # 비동기적으로 파일 읽기
    audio = AudioSegment.from_file(io.BytesIO(audio_bytes), format="m4a")

    # WAV로 변환된 파일을 BytesIO로 저장
    wav_io = io.BytesIO()
...
async def speech_to_text(audio_file):
    # 2-1. 오디오 파일 로드
    wav_audio = await convert_m4a_to_wav(audio_file)

    audio, sr = librosa.load(wav_audio, sr=16000)  # 샘플링 레이트 16kHz로 맞춤
...
if __name__ == "__main__":
    audio_file = "clearSpeech.m4a"  # 처리할 오디오 파일 경로
...
# LLM 인스턴스 생성
llm = ChatGoogleGenerativeAI(api_key=API_KEY, model="gemini-1.5-flash", temperature=0.7)
...
def pronunciation_correction(input_data) -> str:
    ...
    while attempt < max_attempts:
        ...
        response = chain2.invoke(
            {"raw_tts": input_data["raw_tts"], "right_sentence": right_sentence_value}
        )
